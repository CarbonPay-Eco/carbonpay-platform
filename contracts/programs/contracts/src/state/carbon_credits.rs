...
impl CarbonCredits {
    ...
    pub fn add_project_credits(&mut self, credits_amount: u64) -> Result<()> {
        ...
        self.total_credits = self.total_credits.checked_add(credits_amount)
            .ok_or(ProgramError::ArithmeticOverflow)?;
        self.active_credits = self.active_credits.checked_add(credits_amount)
            .ok_or(ProgramError::ArithmeticOverflow)?;
        self.projects_count = self.projects_count.checked_add(1)
            .ok_or(ProgramError::ArithmeticOverflow)?;
        Ok(())
    }
    ...
    pub fn record_offset(&mut self, offset_amount: u64) -> Result<()> {
        ...
        self.active_credits = self.active_credits.checked_sub(offset_amount)
            .ok_or(ProgramError::ArithmeticOverflow)?;
        self.offset_credits = self.offset_credits.checked_add(offset_amount)
            .ok_or(ProgramError::ArithmeticOverflow)?;
        Ok(())
    }
    ...
    pub fn add_fees(&mut self, fee_amount: u64) -> Result<()> {
        ...
        self.total_fees_earned = self.total_fees_earned.checked_add(fee_amount)
            .ok_or(ProgramError::ArithmeticOverflow)?;
        Ok(())
    }
}
